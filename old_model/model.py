...
class AttentionInputsOutput(BaseModelOutput):
    ...
    all_skip_hidden_states: List[Tuple[torch.tensor]] = None
...
class FunnelAeEncoder(nn.Module):
    ...
    def forward(
        ...
        for block_index, block in enumerate(self.blocks):
            ...
            if pooling_flag:
                all_skip_hidden_states = all_skip_hidden_states + (hidden,)
...
class FunnelAeDecoder(nn.Module):
    def __init__(self, config) -> None:
        ...
        # TODO get encoder layer indices at the end of each encoder block
        self.encoder_block_indices = []


    def forward(
        self,
        last_hidden_state,
        all_skip_hidden_states=None,
        skip_w=None,
        attention_mask=None,
        token_type_ids=None,
        output_attentions=False,
        ...
        '''
            TODO: Switch to handling attention_inputs independently (for real this time).
            - `position_embeds` is constant
            - The other fields (padding & token type) only require doubling the dimensions
            - - Here info on things like padding will inevitably be lost.
            - - I think it would be fine to not handle padding as I'm only interested in non-padded training anyway.
        '''
        # The pooling is not implemented on long tensors, so we convert this mask.
        ...
        use_skip = skip_w is not None and all_skip_hidden_states is not None
        if use_skip:
            assert skip_w.size(0) == len(self.blocks)
            assert len(all_skip_hidden_states) == len(self.blocks)

        for block_index, block in enumerate(self.blocks):
            upsampling_flag = hidden.size(1) < all_skip_hidden_states[0].size(1)
            upsampling_flag = upsampling_flag and block_index > 0
            if upsampling_flag:
                if use_skip:
                    encoder_hidden = all_skip_hidden_states[-(block_index+1)]
                    hidden = hidden * (1 - skip_w[block_index]) + encoder_hidden * skip_w[block_index]
            ...
            for (layer_index, layer) in enumerate(block):
                for repeat_index in range(self.config.block_repeats[block_index]):
                    ...
                    if do_upsampling:
                        ...
                        query = key = value = hidden
                    # TODO combine & fix `all_attention_inputs` and `upsample_blocks`
                    layer_output = layer(query, key, value, attention_inputs, output_attentions=output_attentions)
                    hidden = layer_output[0]
                    if do_upsampling and self.config.pool_q_only:
                        attention_inputs = self.attention_structure.post_attention_pooling(attention_inputs)
...
class FunnelAeBaseModel(FunnelBaseModel, FunnelAePreTrainedModel):
    ...
    def forward(
        ...
        last_hidden_state, hidden_states, attention_inputs, all_skip_hidden_states = encoder_outputs[:4]

        decoder_outputs = self.decoder(
            last_hidden_state,
            all_attention_inputs=attention_inputs,
            all_skip_hidden_states=all_skip_hidden_states,
            skip_w=skip_w,
            attention_mask=attention_mask,
            token_type_ids=token_type_ids,
            output_attentions=output_attentions,
